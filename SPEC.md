...
**Keepalive ping (`PeerConfig.PingHost` / `PingTimeout` / `PingDownCount` / `PingUpCount`).** Optional in-tunnel IP pinged once per keepalive period (25 s when keepalive is 0); per-ping timeout = `PingTimeout` seconds (1–60, default 3, capped at the period). Keeps handshakes fresh and, when set, decides failover health. Warn (not block) when outside the peer's allowed IPs. When the ping host is unique to its peer (across all connected tunnels) and inside that peer's allowed IPs, Connect pins a `/32` probe route to that adapter — mwan3-style — so the probe tests this path even while the peer is standby; a shared ping host can't be pinned and only counts while the peer's adapter would carry it.

**Failover for overlapping allowed IPs (`PeerConfig.Metric`).** The same CIDR (exact match after masking; a `/0` groups as its two `/1` route halves) may be claimed by peers on one or many tunnels — a *route group*. Per group, members order by (Metric asc, tunnel name, peer index); the best *healthy* member is active. **Metric** is a 0–10 input; connect blocks when group members share a value (`MetricConflict`). **Health, one rule per signal:** ping host set (and the probe actually tests this path) → `PingDownCount` consecutive failures = down, `PingUpCount` consecutive successes = up (each 1–100, default 3; independent so recovery can be judged more cautiously than failure — the counts *are* the hysteresis; legacy `PingCount =` parses as both). No ping host (or untestable probe) → handshake freshness decides: >180 s old = down (a handshake refreshes at most every ~2 min under keepalive), fresh = up; 60 s first-handshake grace after connect. Arbitration, each poll tick: across adapters the active member's route keeps metric 0, every other adapter's route is pushed to ≥400 (`SetIpForwardEntry2`); within one adapter (WireGuard forbids true overlap) the grouped CIDR is assigned to that adapter's best member via live `SetConfiguration`. Dissolved groups restore metric 0. Balancing modes beyond metric failover (per-flow spreading, latency racing) were considered and rejected for now — Windows has no ECMP/policy-NAT without a WFP driver.
...
- **Edit mode** (click to expand; any number of cards may be expanded at once): stacked full-width sections. **Interface** = one line — `Interface` title · public key (readonly, copy) · private key (icon-only generate button inside the box, visible only while empty — nothing to discard, so no confirm) · address chips + add, with the listen port (watermark `port`, no label) docked upper-right. **Peers** = full-width blocks stacking vertically: header (`Peer` · editable public key · preshared · active/standby badge · ping RTT · handshake ⟶ metric docked upper-right, enabled only while an overlap exists) · `Endpoint` line (host stretches · :port · keepalive) · `Allowed IPs` chips + square add · `Ping` sentence (host · down after · up after · timeout s) · `DNS` (icon-only pin: outline dim / filled accent) + label-less domain chips with an "Add domain" placeholder box. Numeric boxes are digit-sized (metric/counts/timeout 2 digits, keepalive 3, ports 5). `+ Add peer` / `Remove peer`; footer `Delete tunnel` (danger, left, two-click confirm) · raw-config toggle (Ctrl+E) · `Cancel` · `Save`.
...
Compiled by `build.ps1` with `/DAppVersion` from `VERSION`; `ArchitecturesAllowed=x64os` (strict — the WireGuardNT driver is native-only, no ARM emulation). `WizardStyle=modern dynamic` — the wizard follows the Windows light/dark system setting. Installs `SplitGuard.exe` + `wireguard.dll` to `{autopf}\SplitGuard`, Start Menu shortcut, optional desktop icon, `PrivilegesRequired=admin`. Install kills a running instance first (`PrepareToInstall` → taskkill). Uninstall: taskkill → `SplitGuard.exe --cleanup` (NRPT + scheduled tasks) → remove files; `%ProgramData%\SplitGuard\config.json` is kept.
