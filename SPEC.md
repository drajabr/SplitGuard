...
- **Edit mode** (click to expand; any number of cards may be expanded at once): stacked full-width sections. **Interface** = one line — `Interface` title · public key (readonly, copy) · private key · address chips + add, with `generate` + listen port docked upper-right. **Peers** = full-width blocks stacking vertically, four lines each: header (`Peer` · editable public key · preshared (masked) · active/standby badge · ping RTT · handshake ⟶ endpoint host:port · keepalive · metric docked upper-right, metric enabled only while an overlap exists) · `Allowed IPs` chips + square add · `Ping` sentence (host · down after · up after · timeout s) · `DNS` (icon-only pin: outline dim / filled accent) + label-less domain chips with an "Add domain" placeholder box. `+ Add peer` / `Remove peer`; footer `Delete tunnel` (danger, left, two-click confirm) · raw-config toggle (Ctrl+E) · `Cancel` · `Save`.
